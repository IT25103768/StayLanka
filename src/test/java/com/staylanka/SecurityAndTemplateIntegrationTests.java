...
class SecurityAndTemplateIntegrationTests {
    ...
    void publicPagesRenderWithoutAuthentication() throws Exception {
        ...
        mockMvc.perform(get("/promotions"))
                .andExpect(status().isOk())
                .andExpect(view().name("promotion/public-list"));

        mockMvc.perform(get("/reviews"))
                .andExpect(status().isOk())
                .andExpect(view().name("review/public-list"));
    ...
    void legacyStaffCannotModifyModuleOwnedRoutes() throws Exception {
        ...
        mockMvc.perform(
                        post("/staff/rooms/999/deactivate")
                                .with(
                                        user("legacy-staff@test.lk")
                                                .roles("STAFF")
                                )
                                .with(csrf())
                )
    ...
    void roomManagerCannotModifyOtherModules() throws Exception {
        ...
        mockMvc.perform(
                        post("/staff/reservations/999/confirm")
                                .with(manager)
                                .with(csrf())
                )
                .andExpect(status().isForbidden());
        ...
        mockMvc.perform(
                        post("/admin/promotions/999/toggle-active")
                                .with(manager)
                                .with(csrf())
                )
                .andExpect(status().isForbidden());
    ...
    void reservationManagerCannotModifyOtherModules() throws Exception {
        ...
        mockMvc.perform(
                        post("/admin/customers/999/toggle-active")
                                .with(manager)
                                .with(csrf())
                )
                .andExpect(status().isForbidden());
    ...
    void promotionReviewManagerCannotModifyOtherModules() throws Exception {
        ...
        var manager =
                user("IT25103767")
                        .roles("PROMOTION_REVIEW_MANAGER");
