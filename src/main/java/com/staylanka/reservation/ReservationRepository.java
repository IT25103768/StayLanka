...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    /**
     * Loads a reservation together with all related data needed
    ...
    /**
     * Finds reservations belonging to a customer by user email.
     */
    @EntityGraph(attributePaths = {
            "room",
            "room.roomType",
    ...
    Page<Reservation> findByCustomerUserEmailIgnoreCase(
            String email,
            Pageable pageable
    );
    ...
    /**
     * Finds reservations that are active during a given date range.
     */
    @EntityGraph(attributePaths = {
            "customer",
            "customer.user",
    ...
    Page<Reservation> findByStatusAndCheckInDateLessThanEqualAndCheckOutDateAfter(
            ReservationStatus status,
            LocalDate latestCheckIn,
            LocalDate earliestCheckOut,
            Pageable pageable
    );

    /**
     * Searches reservations using optional status and date filters.
     */
    @EntityGraph(attributePaths = {
            "customer",
            "customer.user",
    ...
    @Query("""
            ...
            where (:status is null or r.status = :status)
              ...
              and (
                    lower(r.reservationReference) like
                        lower(concat('%', :term, '%'))
                    or lower(r.customer.user.email) like
                        lower(concat('%', :term, '%'))
                    or lower(r.room.roomNumber) like
                        lower(concat('%', :term, '%'))
                  )
            """)
    ...
    /**
     * Counts reservations that overlap with the requested dates.
     *
     ...
     *
     * Cancelled, rejected and no-show reservations do not block the room.
     */
    @Query("""
            select count(r)
            from Reservation r
    ...
    /**
     * Counts reservations that prevent operational room changes.
     */
    @Query("""
            select count(r)
            from Reservation r
            ...
            """)
    long countBlockingOperationalChanges(
            @Param("roomId") Long roomId,
            @Param("today") LocalDate today
    );

    /**
     * Counts active reservations whose guest count exceeds
     * the room type capacity.
     */
    @Query("""
            select count(r)
            from Reservation r
            ...
            """)
    long countCapacityConflictsForRoomType(
